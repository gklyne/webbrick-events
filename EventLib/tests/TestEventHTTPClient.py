...
#
# This performs tests similar to TestEventRouterHTTP, but runs against an
# external running server implemented by TestEventHTTPServer.
# Essentially, this is a confidence test of the external server process, 
# whose real purpose is to provide a test server for client-side javascript 
# event distribution functions.
#
# To run this test, first start the test server 'TestEventHTTPServer.py' on
# the same host, then run this program.
#
...
# Test class
class TestEventHTTPClient(unittest.TestCase):
    ...
    def tearDown(self):
        Trace("TearDown", "EventLib.TestEventHTTPClient")
    ...
    def doSubscriptionForwardingR2R3(self, evtype, source, evmatch, evdrop, r1fwd=0):
        self.setUpClient()
        self.doSubscriptionForwarding(self.R2C, self.R3C, evtype, source, evmatch, evdrop, delay=0.2)
        return
    ...
    def testTestEventHTTPClientIntro(self):
        Info("---- testTestEventHTTPClientIntro: TestEventHTTPClient takes about 100s to run","EventLib.TestEventHTTPClient")
    ...
    def testSubscriptionForwarding1(self):
        Info("---- testSubscriptionForwarding1 ----","EventLib.TestEventHTTPClient")
    ...
    def testSubscriptionForwarding2(self):
        Info("---- testSubscriptionForwarding2 ----","EventLib.TestEventHTTPClient")
        evmatch = makeEvent(evtype="R3Events/ev1",source="R3Source/src1")
        evdrop  = makeEvent(evtype="R3Events/ev2",source="R3Source/src2")
    ...
    def testTestEventHTTPClientDone(self):
        Info("---- testTestEventHTTPClientDone ----","EventLib.TestEventHTTPClient")
