...
class TestFunctions(unittest.TestCase):
    ...
    def testCwd(self):
        ...
        cwd = os.getcwd() 
        expected = LibName+os.sep+"tests"
        # when run as global test suite
        expected2 = "WebBrickLibs"
        assert endsWith(cwd, expected) or endsWith(cwd, expected2), "Cwd is %s expected %s" % (cwd,expected)
